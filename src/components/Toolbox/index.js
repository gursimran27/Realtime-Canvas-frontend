...
const Toolbox = () => {
    ...
    const updateBrushSize = (e) => {
        ...
        socket.emit('changeConfig', {color, size: e.target.value, roomId: roomId })
    }

    const updateColor = (newColor) => {
        dispatch(changeColor({item: activeMenuItem, color: newColor}))
        socket.emit('changeConfig', {color: newColor, size, roomId: roomId })
    }
    ...
    return (<div className={styles.toolboxContainer}>
        ...
        {showBrushToolOption && <div className={styles.toolItem}>
            ...
            <div className={styles.itemContainer}>
                <input className=' cursor-pointer' type="range" min={1} max={10} step={1} onChange={updateBrushSize} value={size}/>
            </div>
        </div>}
    </div>)
}
